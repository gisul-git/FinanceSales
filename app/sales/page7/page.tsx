...
const skillGrowth = [
  { skill: "Product",     before: 48, after: 85 },
  { skill: "Negotiation", before: 35, after: 80 },
  { skill: "CRM",         before: 40, after: 88 },
  { skill: "Comms",       before: 52, after: 86 },
  { skill: "Objections",  before: 30, after: 78 },
  { skill: "Pipeline",    before: 38, after: 84 },
];
...
const metricRows: { icon: React.ReactNode; label: string; before: string; after: string; highlight?: boolean }[] = [
  { icon: <BarChart2 size={16} />,     label: "Aaptor Score",         before: "374",    after: "726",      highlight: true },
  ...
  { icon: <BarChart2 size={16} />,     label: "Deals Closed (Qtr)",   before: "3",      after: "9"         },
  { icon: <Clock size={16} />,         label: "Avg Deal Cycle",       before: "28 days",after: "18 days"   },
  { icon: <BookOpen size={16} />,      label: "Learning Hours",       before: "0 hrs",  after: "44 hrs"    },
  { icon: <Zap size={16} />,           label: "Active Streak",        before: "—",      after: "19 days"   },
];
...
export default function SalesPage7() {
  return (
    <DemoLayout persona="sales" currentPage={7}>
      <motion.div variants={stagger} initial="hidden" animate="show"
        ...
        <motion.div variants={fadeUp} className="rounded-2xl px-6 py-6"
          ...
          <p className="text-xs uppercase tracking-widest mb-2" style={{ color: "var(--text-muted)", fontFamily: "Syne, sans-serif" }}>Transformation Summary</p>
          <h1 className="text-2xl font-bold mb-4" style={{ fontFamily: "Syne, sans-serif", color: "var(--text-primary)" }}>
            Neha's Sales Transformation in <span style={{ color: "var(--accent-mint)" }}>6 Months on Aaptor</span>
          </h1>
          <div className="flex flex-wrap gap-4">
            {[{ label: "Aaptor Score", from: "374", to: "726", delta: "+352 pts" }, { label: "Overall Sales Level", from: "40%", to: "83%", delta: "+43%" }].map((s) => (
        ...
        <motion.div variants={fadeUp} whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 24 }} viewport={{ once: true }}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={skillGrowth} layout="vertical" margin={{ top: 8, right: 40, left: 8, bottom: 0 }}>
              ...
              <Legend wrapperStyle={{ fontSize: 12, color: "var(--text-secondary)" }} formatter={(v) => v === "before" ? "Before" : "After"} />
        ...
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 24 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={scoreJourney} margin={{ top: 20, right: 16, left: -10, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="score" stroke="#2dcc70" strokeWidth={2} fill="url(#salesGrad)"
                dot={(props) => {
        ...
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 24 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          ...
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
            Neha has shown <span style={{ color: "var(--accent-mint)" }}>43% overall skill improvement</span>. Her deal cycle reduced by <span style={{ color: "var(--accent-mint)" }}>36%</span>, directly impacting revenue velocity. Ranked <span style={{ color: "var(--accent-green)" }}>Top 15% in her cohort</span>. Ready for Senior Account Executive engagements.
          </p>
          <div className="flex flex-wrap gap-2">
            {[{ icon: <Trophy size={12} />, label: "Top 15% in cohort" }, { icon: <CheckCircle2 size={12} />, label: "Salesforce Certified" }, { icon: <Shield size={12} />, label: "Top Closer" }].map((badge) => (
