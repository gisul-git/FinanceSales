...
const skills = [
  { label: "Product Knowledge",           level: 48 },
  { label: "Negotiation & Closing",       level: 35 },
  { label: "CRM Proficiency",             level: 40 },
  { label: "Communication & Presentation",level: 52 },
  { label: "Objection Handling",          level: 30 },
  { label: "Pipeline Management",         level: 38 },
];
...
export default function SalesPage1() {
  return (
    <DemoLayout persona="sales" currentPage={1}>
      <motion.div variants={stagger} initial="hidden" animate="show"
        ...
        <motion.div variants={fadeUp} className="flex items-start gap-3 rounded-xl px-4 py-3"
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#fca5a5" }}>
            Neha's sales competencies are unverified. No benchmark data available.
          </p>
        </motion.div>
        ...
        <motion.div variants={fadeUp} className="rounded-2xl p-5 flex items-center gap-5"
          ...
          <div className="w-16 h-16 rounded-full flex items-center justify-center text-lg font-bold flex-shrink-0"
            style={{ fontFamily: "Syne, sans-serif", background: "rgba(245,158,11,0.12)", border: "2px solid #f59e0b", color: "#fbbf24" }}>
            NK
          </div>
            ...
            {skills.map((item, i) => (
              <div key={item.label} className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-semibold" style={{ color: skillColor(item.level), fontFamily: "Syne, sans-serif" }}>{item.level}%</span>
                </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <motion.div variants={fadeUp} className="rounded-2xl p-6 flex flex-col items-center justify-center gap-2.5 text-center"
            ...
            <p className="text-sm font-semibold" style={{ color: "var(--text-muted)", fontFamily: "Syne, sans-serif" }}>No Learning History</p>
            <p className="text-xs leading-relaxed" style={{ color: "var(--border)" }}>No courses, certifications, or learning activity recorded.</p>
          </motion.div>
          <motion.div variants={fadeUp} className="rounded-2xl p-6 flex flex-col items-center justify-center gap-2.5 text-center"
            ...
            <p className="text-xs leading-relaxed" style={{ color: "var(--text-muted)" }}>Sales competencies cannot be benchmarked without an assessment.</p>
          </motion.div>
        </div>
      </motion.div>
    </DemoLayout>
